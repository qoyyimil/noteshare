...
class _StatisticsScreenState extends State<StatisticsScreen>
  ...
  Map<String, double> _calculateAxisDetails(double maxDataValue) {
    if (maxDataValue <= 0) {
      ...
      return {
        'maxY': ((maxDataValue / 10).ceil() * 10.0) + 10,
        'interval': 10.0
      };
    } else {
      double interval =
          pow(10, (log(maxDataValue) / log(10)).floor()).toDouble();
      return {
        'maxY': ((maxDataValue / interval).ceil() * interval) + interval,
        'interval': interval / 2
      };
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<SearchProvider>(
        ...
        child: Column(
          // Menggunakan Column untuk menampung TabBar dan TabBarView
          children: [
            ...
            Expanded(
              ...
              child: Padding(
                padding: const EdgeInsets.symmetric(
                    horizontal: 32.0,
                    vertical: 8.0), // Tambahkan padding di sini
                child: TabBarView(
                  controller: _tabController,
                  children: [
    ...
    return StreamBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        for (var entry in sortedEntries) {
          ...
          readsSpots.add(
              FlSpot(entry.key.millisecondsSinceEpoch.toDouble(), readValue));
          likesSpots.add(
              FlSpot(entry.key.millisecondsSinceEpoch.toDouble(), likeValue));
        }
            ...
            leftTitles: AxisTitles(
              ...
              getTitlesWidget: (value, meta) {
                return Text(value.toInt().toString(),
                    style: TextStyle(color: subtleTextColor, fontSize: 10));
              },
            )),
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                interval: spots.length > 5
                    ? Duration(days: 5).inMilliseconds.toDouble()
        ...
        return StreamBuilder<int>(
          ...
          builder: (context, followingSnapshot) {
            if (followersSnapshot.connectionState == ConnectionState.waiting ||
                followingSnapshot.connectionState == ConnectionState.waiting) {
              return const Center(child: CircularProgressIndicator());
            }
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 32), // Jarak antar card
                        _buildStatsCard("Following", following.toString()),
                      ],
                    ),
                    const SizedBox(height: 32),
                    Text("More audience analytics coming soon!",
                        style: GoogleFonts.lato(color: subtleTextColor)),
                  ],
                ),
              ),
